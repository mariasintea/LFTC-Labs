 ...
 */
bool hasOnlyLetters(string word, AF af) {
    return af.checkSequence(word) && word != "return" && word.length() < 250;
}
 ...
 * @param word - stringul dat
 */
bool hasOnlyDigits(string word, AF af) {
    return af.checkSequence(word);
}
 ...
 */
void checkForConstsAndIds(map<string, int> &constants, map<string, int> &identifiers, bool &foundCONST, bool &foundID, string givenLine, AF ids, AF consts){
    size_t pos = 0;
    string word;
    ...
    while ((pos = givenLine.find(" ")) != std::string::npos)
    {
        ...
        if(word.length() > 0){
            if (hasOnlyDigits(word, consts))
                constants.insert(pair<string, int> (word, constants.size()));
            else
                if (hasOnlyLetters(word, ids))
 ...
 */
void findElements(map<string, int> &atoms, map<string, int> &constants, map<string, int> &identifiers, AF ids, AF consts){
    ...
    while (!fin.eof()){
        ...
        checkForConstsAndIds(constants, identifiers, foundCONST, foundID, line, ids, consts);
 ...
 */
int createFIP(vector<pair<int, int>> &fip, map<string, int> &atoms, map<string, int> &constants, map<string, int> &identifiers, AF ids, AF consts) {
    ifstream fin("SOURCE.txt");
    string line;
                ...
                if (word == "if" || word == "while") { // if (conditie) { si while(conditie) {
                    ...
                    if (word[0] == '(') { // prima expresie din conditie
                        ...
                        while (!word.empty() && word != "==" && word != ">=" && word != "<=" && word != ">" && word != "<") {
                            if (hasOnlyLetters(word, ids))
                                fip.push_back(pair<int, int>(atoms["ID"], identifiers[word]));
                            else if (hasOnlyDigits(word, consts))
                    ...
                    if (word == "else") {
                        ...
                        if (hasOnlyLetters(word, ids)){ // operatia de atribuire: ID = expresie
                            fip.push_back(pair<int, int>(atoms["ID"], identifiers[word]));
                            line.erase(0, word.length() + 1);
                            word = line.substr(0, line.find(" "));
                            if (word == "=") {
...
void readDataFromFile(vector<string> &alphabet, vector<string> &states, string &initialState,
                      vector<Transition> &transitions, vector<string> &finalStates, string fileName){
    string line;
    int n;
...
AF readAF(string fileName){
    ...
    readDataFromFile(alphabet, states, initialState, transitions, finalStates, fileName);
    return AF(alphabet, states, initialState, transitions, finalStates);
}
...
int main() {
    ...
    AF AFConstants = readAF("constante.txt");
    AF AFIdentifiers = readAF("identificatori.txt");

    findElements(atoms, constants, identifiers, AFIdentifiers, AFConstants);
    int line = createFIP(fip, atoms, constants, identifiers, AFIdentifiers, AFConstants);
    if (line == -1)
